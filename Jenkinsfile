...
pipeline{
    ...
    stages{        
      ...
      stage ('Deploying War File'){
         ...
         post{
   success {
     sh 'curl -H "Content-Type: application/json" -X POST -d \'{"team_name":'${KSR}',"jobtitle":'${JOB_NAME}',"bNumber":'${BUILD_NUMBER}',"bUrl":'${BUILD_URL}',"buildStatus":"SUCCESS"}\'  http://ec2-13-232-255-41.ap-south-1.compute.amazonaws.com:8080/Backend_XFS/addbuildinfo'
   }
         failure {
      sh 'curl -H "Content-Type: application/json" -X POST -d \'{"team_name":'${KSR}',"jobtitle":'${JOB_NAME}',"bNumber":'${BUILD_NUMBER}',"bUrl":'${BUILD_URL}',"buildStatus":"FAILURE"}\'  http://ec2-13-232-255-41.ap-south-1.compute.amazonaws.com:8080/Backend_XFS/addbuildinfo'
